...
var slotMachine = function (el, options, track) {
    ...
    slot.endSpin = function () {
        if (slot.options.endNum == 0) {
            slot.options.endNum = 0;// slot.randomRange(1, slot.liCount);
            // slot.$el.append('<li>0</li>');
        }
        ...
        var finalSpeed;
        if (slot.options.endNum == 0) {
            finalSpeed = ((slot.spinSpeed * 1.5) * (slot.liCount)) / 1;
        } else {
            finalSpeed = ((slot.spinSpeed * 1.5) * (slot.liCount)) / slot.options.endNum;
        }
        

        finalPos = finalPos - slot.liHeight;

        console.log('slot.liHeight '+slot.liHeight);
        console.log('slot.options.endNum '+slot.options.endNum);
        console.log('slot.liHeight '+slot.liHeight);
